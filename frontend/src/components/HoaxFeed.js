...
const HoaxFeed = () => {
  ...
  return (
    <div>
      {newHoaxCount > 0 && (
        <div
          className="alert alert-primary text-center mb-1"
          style={{ cursor: loadNewHoaxesProgress ? "wait" : "pointer" }}
      ...
      {!last && (
        <div
          ...
          style={{ cursor: loadOldHoaxesProgress ? "wait" : "pointer" }}
          onClick={loadOldHoaxesProgress ? () => {} : loadOldHoaxes}
        >
          {loadOldHoaxesProgress ? <Spinner /> : t("Load More..")}
        </div>
      )}
    </div>
  );
